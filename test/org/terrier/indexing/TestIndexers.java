...
public class TestIndexers extends ApplicationSetupBasedTest {
	...
	protected void testIndexer(Indexer indexer, boolean directExpected, boolean fieldsExpected) throws Exception {
		 ...
		 */
		PostingIndex<Pointer> invertedIndex = index.getInvertedIndex();
...
//			assertNotNull(le);
//			
//			int[][] documents = ((InvertedIndex)invertedIndex).getDocuments(le);
//			
//			if (!fieldsExpected) {
...
//		}		
		...
		if (directExpected) {
			 ...
			 */
			PostingIndex<Pointer> directIndex = index.getDirectIndex();
...
//				assertNotNull(de);
//				
//				int[][] terms = ((DirectIndex) directIndex).getTerms(de);
//				
//				if (!fieldsExpected) {
